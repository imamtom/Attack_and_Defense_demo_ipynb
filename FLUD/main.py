...
from datasets import AGNewsDataset, AGNewsDataset_server, AGNewsDataset_per_client
import copy
import torch
import numpy as np
...
def run(attack_method, defense_method, num_clients, poisoned_client_portion, poison_data_portion, dataset_name, gpu_id, iid, alpha, global_rounds, local_epochs, local_learning_rate, local_momentum, batch_size, given_size, target_label):
    ...
    if dataset_name == "ImageNet12":
        all_dataset_train = ImagenetteDataset(root_dir='/home/xd/lwj/imagenet12/train')
        all_dataset_val = ImagenetteDataset(root_dir='/home/xd/lwj/imagenet12/val')

    elif dataset_name == "AgNews":
        all_dataset_train = AGNewsDataset(root_dir='/home/xd/lwj/AGNews/train.csv')
        all_dataset_val = AGNewsDataset(root_dir='/home/xd/lwj/AGNews/test.csv')
    ...
    # 初始化每个客户端的train_loader, 以及server的test_loader
    if dataset_name == "AgNews":
        train_loaders = [torch.utils.data.DataLoader(client_dataset_instances[i], batch_size=batch_size, collate_fn=collate_fn, shuffle=True, num_workers = num_workers_for_dataloader) for i in range(num_clients)]
        eval_acc_dataset_instance = all_dataset_val.to_server_dataset(target_label=None)
        eval_asr_dataset_instance = all_dataset_val.to_server_dataset(target_label=target_label)
        test_acc_loader = torch.utils.data.DataLoader(eval_acc_dataset_instance, batch_size=batch_size, collate_fn=collate_fn, shuffle=False, num_workers = num_workers_for_dataloader)
        test_asr_loader = torch.utils.data.DataLoader(eval_asr_dataset_instance, batch_size=batch_size, collate_fn=collate_fn, shuffle=False, num_workers = num_workers_for_dataloader)
    ...
    for round in range(global_rounds):
        ...
        asr = test_acc(global_model, test_asr_loader, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='AgNews', help='Name of the dataset')
